 ...
 @RunWith(classOf[JUnitRunner])
  ...
  test("max of an empty list ") {
    ...
    try {
      ...
      case _: NoSuchElementException => println("Test passed")
  ...
  test("sum with negatives ") {
    ...
    assert(sum(List(1, 2, 4, -2, -1)) === 4)

  }
  test("Max with duplicates"){
    assert(max(List(1,2,4,4,18,18,120,120)) === 120)
  }
  test("Max with negatives"){
    assert(max(List(1,2,4,4,18,18,120,-120, 2002, 22200123, -1)) === 22200123)
  }
  test("Sum with duplicates"){
    assert(sum(List(1,1,1,1,1,1,1,1,12,21)) === 41)
  }
